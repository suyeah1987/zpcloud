...
Mod_edit_grub(){
echo "#####################################update grub##############################/n"
sed -i  's/us/& biosdevname=1/' /etc/grub.conf
sed -i  's/quiet/& console=tty0 console=ttyS0,115200/' /etc/grub.conf
sed -i  's/us/& biosdevname=1/' /boot/grub/grub.conf
sed -i  's/quiet/& console=tty0 console=ttyS0,115200/' /boot/grub/grub.conf
...
wget -O /etc/sysconfig/network-scripts/ifup-eth http://download.tcwyun.com/images/packer/packer_file_6.9/ifup-eth
